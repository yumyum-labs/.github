# 🥗 MyEat (마잇) - Your Personal AI Diet Coach

> **"건강한 식습관, 따뜻한 공감과 함께."**  
> MyEat은 사용자의 감정을 이해하고, 데이터에 기반한 정밀한 식단 솔루션을 제공하는 AI 코칭 플랫폼입니다.

<div align="center">

[![Tech Stack](https://img.shields.io/badge/Tech%20Stack-Spring%20Boot%20%7C%20React-green)](https://github.com/yumyum-labs)
[![AI Powered](https://img.shields.io/badge/AI-OpenAI%20GPT--4o--mini-blue)](https://openai.com/)
[![License](https://img.shields.io/badge/License-MIT-yellow)]()

</div>

---

## 🚀 Project Overview

**MyEat**은 단순한 칼로리 계산 앱이 아닙니다.  
MBTI 페르소나 기반의 AI 챗봇이 사용자와 1:1로 대화하며, 정서적 지지와 함께 실행 가능한 식단 가이드를 제공합니다.

### ✨ Key Features
- **🧠 Persona AI Coaching:** ENFJ, ISTJ 등 MBTI 성향에 맞춘 맞춤형 대화 톤 앤 매너 제공.
- **📊 Analytical & Emotional Balance:** G-Eval v4.0 평가 기준을 통과한 **감성 공명**과 **데이터 정밀도**의 완벽한 조화.
- **🛡️ Robust Architecture:** 
    - Cloudflare WAF 우회를 위한 **OkHttp + Curl Fallback** 하이브리드 클라이언트 적용.
    - **Facade Pattern**을 통한 유연한 API 연동 구조.
- **📝 Smart Memory Summary:** 대화 맥락을 기억하고 요약하여 지속적인 케어 제공.

---

## 🛠️ Tech Stack

### Backend & AI
<img src="https://img.shields.io/badge/Java-17-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"> <img src="https://img.shields.io/badge/Spring%20Boot-3.5-6DB33F?style=for-the-badge&logo=springboot&logoColor=white">
<img src="https://img.shields.io/badge/MyBatis-3.0-000000?style=for-the-badge&logo=mybatis&logoColor=white"> <img src="https://img.shields.io/badge/OkHttp-4.12-000000?style=for-the-badge&logo=square&logoColor=white"> <img src="https://img.shields.io/badge/OpenAI-GPT--4o--mini-412991?style=for-the-badge&logo=openai&logoColor=white">

### Frontend
<img src="https://img.shields.io/badge/React-18.3-61DAFB?style=for-the-badge&logo=react&logoColor=black"> <img src="https://img.shields.io/badge/Vite-6.3-646CFF?style=for-the-badge&logo=vite&logoColor=white"> <img src="https://img.shields.io/badge/Radix%20UI-161618?style=for-the-badge&logo=radixui&logoColor=white">

### Data & Infra
<img src="https://img.shields.io/badge/MySQL-8.0-4479A1?style=for-the-badge&logo=mysql&logoColor=white"> <img src="https://img.shields.io/badge/Redis-Cache-DC382D?style=for-the-badge&logo=redis&logoColor=white"> <img src="https://img.shields.io/badge/Docker-Container-2496ED?style=for-the-badge&logo=docker&logoColor=white"> <img src="https://img.shields.io/badge/Git-VCS-F05032?style=for-the-badge&logo=git&logoColor=white">

---

## 🎨 Design Resource

| Platform | Link | Description |
| :--- | :--- | :--- |
| **Figma** | [**View Design Mockups**](#) | UI/UX 디자인 목업 및 프로토타입 (Link to be updated) |

---

## 📂 Repositories

| Repository | Tech Stack | Description |
| :--- | :--- | :--- |
| **[yumyum-be](https://github.com/yumyum-labs/yumyum-be)** | `Spring Boot` `MyBatis` `Redis` | 메인 백엔드 서버. DB 연동 및 비즈니스 로직 처리. |
| **[yumyum-ai-dev](https://github.com/yumyum-labs/yumyum-ai-dev)** | `Spring Boot` `OpenAI` `OkHttp` | AI 챗봇 코어 모듈. 프롬프트 엔지니어링 (GPT-4o-mini), G-Eval 품질 평가. |
| **[yumyum-config](https://github.com/yumyum-labs/yumyum-config)** | `YAML` | 마이크로서비스 및 애플리케이션 설정 중앙 관리 저장소. |
| **[yumyum-workflows](https://github.com/yumyum-labs/yumyum-workflows)** | `GitHub Actions` | CI/CD 파이프라인 및 자동화 워크플로우 스크립트 모음. |

---

## 🏆 Quality Assurance (G-Eval)

우리는 AI 응답의 품질을 보장하기 위해 자체적인 평가 시스템(**G-Eval v4.0**)을 운영합니다.

- **평가 항목:** 공감 능력, 데이터 정확성, 실행 유도력, 메모리 활용 등 9개 축
- **평균 점수:** 4.01 / 5.0 (2025.11 기준)
- **안전성:** 알레르기 및 질환 정보에 대한 엄격한 필터링 적용

---

## 👥 Team 태선 (SSAFY 14기 서울)

<div align="left">
  <a href="https://github.com/tellang">
    <img src="https://github.com/tellang.png" width="60px" alt="tellang" style="border-radius: 50%"/>
  </a>
  <a href="https://github.com/hy5sun">
    <img src="https://github.com/hy5sun.png" width="60px" alt="hy5sun" style="border-radius: 50%"/>
  </a>
  <!-- 팀원 추가 가능 -->
</div>

---

<div align="center">
  <sub>Designed with by Team 태선</sub>
</div>